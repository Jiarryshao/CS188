...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for livingGhost in livingGhostPositionDistributions:
            mostBelief = 0
            beliefPos = tuple()
            for pos in livingGhost:
                if livingGhost[pos] > mostBelief:
                    mostBelief = livingGhost[pos]
                    beliefPos = pos
            curDis = self.distancer.getDistance(beliefPos, pacmanPosition)
            if curDis < closestDis:
                closestDis = curDis
                closestPos = beliefPos

        choseAction = legal[0]
        sucClosestDis = 99999
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            sucDis = self.distancer.getDistance(successorPosition, closestPos)
            if sucDis < sucClosestDis:
                sucClosestDis = sucDis
                choseAction = action
        return choseAction
        # raiseNotDefined()
        "*** END YOUR CODE HERE ***"
