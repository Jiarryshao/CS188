...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for pos1 in pos:
        for pos2 in pos:
            noisydis=[]
            if manhattanDistance(pos1,pos2) not in mdistance:
                noisydis.append(manhattanDistance(pos1,pos2))
            for noisy in range(1,MAX_NOISE+1):
                if manhattanDistance(pos1,pos2)+noisy not in mdistance:
                    noisydis.append(manhattanDistance(pos1,pos2)+noisy)
                if manhattanDistance(pos1,pos2)-noisy>0:
                    if manhattanDistance(pos1,pos2)-noisy not in mdistance:
                        noisydis.append(manhattanDistance(pos1,pos2)-noisy)
            mdistance.extend(noisydis)
    variableDomainsDict[OBS0] =mdistance
    variableDomainsDict[OBS1] =mdistance
    #raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for elimination in eliminationOrder:
            current,joined=joinFactorsByVariable(current, elimination)
            if len(joined.unconditionedVariables())!=1:
                current.append(eliminate(joined,elimination))
        return normalize(joinFactors(current))
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            if noisyDistance is None:
                return 1
            return 0
        positiondistance = manhattanDistance(pacmanPosition, ghostPosition)
        return busters.getObservationProbability(noisyDistance, positiondistance)
        raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            observProb = self.getObservationProb(observation, gameState.getPacmanPosition(), position, self.getJailPosition())
            self.beliefs[position] *= observProb
        # raiseNotDefined()
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            ...
            for p in newPosDist:
                newBeliefs[p] += newPosDist[p] * self.beliefs[position]
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        aggregateDistribution = DiscreteDistribution()
        for particle in self.particles:
            aggregateDistribution[particle] += self.getObservationProb(observation, gameState.getPacmanPosition(), particle, self.getJailPosition())
        aggregateDistribution.normalize()

        isAllZero = True
        for particle in self.particles:
            if aggregateDistribution[particle] != 0:
                isAllZero = False
                break
        ...
        if isAllZero:
            ...
            newParticles = list()
            for i in range(self.numParticles):
                newParticle = aggregateDistribution.sample()
